...
int actionRightTaken = -1; // Right directional Button
int actionLeftTaken = -1; // Left directional Button
int actionUpTaken = -1; // Up Button
int actionDownTaken = -1; // Down Button
...
/*
	A function to process any output
...
void processGameControllerOutput()
{
	...
	if (actionLeftTaken == -1 && gController.processGameControllerAction() == LEFT_BUTTON_DIRECTION)
	{   	
		// Left
		if (gController.processGameControllerAction() == LEFT_BUTTON_DIRECTION)
		{
	...
	else if (actionLeftTaken == 1 && gController.processGameControllerAction() == LEFT_BUTTON_DIRECTION)
	{
		actionLeftTaken = -1;
	}
	...
	if (actionRightTaken == -1 && gController.processGameControllerAction() == RIGHT_BUTTON_DIRECTION)
	{
   		
		if (gController.processGameControllerAction() == RIGHT_BUTTON_DIRECTION)
		{
			if(!needPromote && !checkMate && !verify && inGame && !board_rotating) 
	...
	else if (actionRightTaken == 1 && gController.processGameControllerAction() == RIGHT_BUTTON_DIRECTION)
	{
		actionRightTaken = -1;
	}
	...
	/////////////////////////////////
   	///  UP BUTTON DIRECTION      ///
   	/////////////////////////////////
   	if (actionUpTaken == -1 && gController.processGameControllerAction() == UP_BUTTON_DIRECTION)
   	{
   	
   		if (gController.processGameControllerAction() == UP_BUTTON_DIRECTION)
   		{
  			if(!needPromote && !checkMate && !verify && inGame && !board_rotating) 
   	...
   	else if (actionUpTaken == 1 && gController.processGameControllerAction() == UP_BUTTON_DIRECTION)
   	{
   		actionUpTaken = -1;
   	}
	...
	///////////////////////////////////
   	///  DOWN BUTTON DIRECTION      ///
   	///////////////////////////////////  	
   	if (actionDownTaken == -1 && gController.processGameControllerAction() == DOWN_BUTTON_DIRECTION)
   	{
    	//cout << "Down pressed" << endl;
    	
    	// Down
   		if (gController.processGameControllerAction() == DOWN_BUTTON_DIRECTION)
   		{
  			if(!needPromote && !checkMate && !verify && inGame && !board_rotating) 
   	...
   	else if (actionDownTaken == 1 && gController.processGameControllerAction() == DOWN_BUTTON_DIRECTION)
   	{
   		actionDownTaken = -1;
   	}
	...
	///////////////////////////////////
   	///  A BUTTON                   ///
   	///////////////////////////////////
   	if (aButtonPressed == -1 && gController.processGameControllerAction() == A_BUTTON)
   	{
    	
		// A button pressed
   		if (gController.processGameControllerAction() == A_BUTTON)
   		{
   			if(!needPromote && !checkMate && !verify && inGame && !board_rotating)
           	{
               	if(selected)
   	...
   	else if (aButtonPressed == 1 && gController.processGameControllerAction() == A_BUTTON)
   	{
   		aButtonPressed = -1;
   	} 	
}
...
// End Game Button
int endGame_pt1_x = 352;
int endGame_pt1_y = 297;

int endGame_pt2_x = 406;
int endGame_pt2_y = 297;

int endGame_pt3_x = 352;
int endGame_pt3_y = 315;

int endGame_pt4_x = 406;
int endGame_pt4_y = 315;
	...
	switch(button)
	{
		case GLUT_LEFT_BUTTON:
			if (state == GLUT_UP)
			{
				...
				// Exit Program Button
				if ((x >= endGame_pt1_x) && (x <= endGame_pt2_x) && (y >= endGame_pt1_y) && (y <= endGame_pt4_y))
				{					
					//if(!inGame)
					//	newGame();
            		//else verify = true;
...
void specialFunction(int key, int x, int y)
{
	...
	if (gController.processGameControllerAction() == 14)
	{
		if(!needPromote && !checkMate && !verify && inGame && !board_rotating) 
			key_A_pressed(chess->getTurnColor());
	}
	...
	if (gController.processGameControllerAction() == 12 && actionRightTaken == -1) 
	{
		if(!needPromote && !checkMate && !verify && inGame && !board_rotating) 
			key_D_pressed(chess->getTurnColor());
	}
...
int main(int argc, char *argv[])
{
    ...
    int horizRes = (glutGet(GLUT_SCREEN_WIDTH) - WINDOW_WIDTH);
    int vertRes = (glutGet(GLUT_SCREEN_HEIGHT) - WINDOW_HEIGHT);

    glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGB | GLUT_DEPTH);
    glutInitWindowSize(WINDOW_WIDTH, WINDOW_HEIGHT);
    glutInitWindowPosition(horizRes/2, vertRes/2);
    glutCreateWindow("Chess");
    
    cout << "Library enabled: " << gController.isLibEnabled() << endl;
    cout << "Game Controller Detected: " << gController.GC_Connected() << endl;
